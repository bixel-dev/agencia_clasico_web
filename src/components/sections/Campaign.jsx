...
function useOembed(id) {
  const [meta, setMeta] = useState(oembedCache[id] || null)
  useEffect(() => {
    if (oembedCache[id]) { setMeta(oembedCache[id]); return }
    let alive = true
...
function CampaignCard({ c, onOpen }) {
  ...
  const title = meta?.title || 'Campaña'
...
export default function Campaign() {
  ...
  const goRel = (delta) => {
    ...
    setOpen(CAMPAIGNS[(i + delta + CAMPAIGNS.length) % CAMPAIGNS.length])
  }
    ...
    <section id="campaign" data-screen-label="07 Campaign" className="section-pad section-dark">
      <div className="container">
        <div className="campaign-header">
          ...
          <p className="muted" style={{ fontSize: 14, maxWidth: 380, lineHeight: 1.55 }}></p>
        </div>
